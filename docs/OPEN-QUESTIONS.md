...
| O27 | resolved — D1 reaffirmed | No. Dan (2026-08-07): stick with Tone.js; `docs/archive/prompts/superseded/DISSONANCE_AUDIO_THREADS.md`'s (formerly `docs/intake/`) Babylon-owned-bus-tree proposal is rejected. Same session also fixed a real D1 violation found in the process: `apps/world` had production hardcoded to Babylon's own Audio Engine V2 (`WorldAudioStack.ts`'s `resolveWorldAudioEngine`/`createWorldAudioStack`), with Tone.js reachable only via a dev-only `?audioEngine=tone` flag — backwards from D1. Flipped: Tone is now the unconditional default in both dev and prod; `?audioEngine=babylon` remains as a dev-only legacy comparison toggle. `apps/museum/dont-turn-around` already used Tone-only and needed no change. | none |

...
**Status:** complete 2026-07-30 — authored profile, deterministic tests, durable reload continuity, and Dev Lineglass JSON round-trip landed.
**Scope boundary:** the witnessed strike + recovery. The emitter build, the mount, the interference verb, control model, animal-mapping, and loss condition are **explicitly out of scope** (deferred to v2 / T32). This doc must not implement, stub-toward, or assume any of them beyond leaving a clean hand-off marker.
**Depends on:** T21 (time + weather) strike/weather target-swap must exist. Nothing else.
**Owning package:** Dissonance app scope + `@dissonance/world` weather (strike event as an authored weather command). No engine fork.
...
1. **Confirm T21 exists and expose its seam.** Locate the weather system (`@dissonance/world`, `WeatherProfile`, `WeatherState`, the target-swap / `requestWeather`-equivalent). Confirm rain intensity is a readable scalar and that a storm/rain target can be commanded programmatically. **If T21 is not yet landed, STOP** and report — this doc cannot proceed without it.
...
**Out-of-bounds paths (do not modify):** shared drone behavior code, `@dissonance/audio` master chain / bus definitions, `applyProfile()` / resolver internals, any pursuer brain code, `EnvironmentProfile` schema. All strike behavior lands as additive data + one authored event consumer.
...
StrikeAnchor {
  ...
  losProbeOffset: Vec3           // authored offset from the live drone position
...
DORMANT:
  ...
  - each tick: test Milo→(live drone position + anchor.losProbeOffset) LOS AND distance <= losRange
...
## Out of scope (do not implement — v2 / T32)
